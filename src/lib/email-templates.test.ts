...
describe("every client email", () => {
  it("has both an HTML and a plain-text version", () => {
    for (const e of clientEmails()) {
      assert.ok(e.html.length > 0, `${e.subject}: missing HTML`);
      assert.ok(e.text.length > 0, `${e.subject}: missing text alternative`);
    }
  });
...
describe("appointment emails", () => {
  ...
  it("tells the client how to turn reminders off", () => {
    assert.match(reminder24h(when).html, /turn these reminders off/i);
    assert.match(reminder24h(when).text, /turn these reminders off/i);
  });
});
