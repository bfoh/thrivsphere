 ...
 *
 * Application code should import from `src/db` instead of here, which adds the
 * `server-only` guard. This module exists without that guard so standalone
 * Node scripts (migrations, seeding) can connect too — `server-only` is a
 * bundler construct and throws under plain `tsx`.
 */
